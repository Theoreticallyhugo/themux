...
main()
{
  ...
  show_left_icon_static=$(get_tmux_option "@dracula-show-left-icon-static" "")
  show_left_icon=$(get_tmux_option "@dracula-show-left-icon" "#S")
  ...
  for plugin in "${plugins[@]}"; do
    ...
    if case $plugin in custom:*) true;; *) false;; esac; then
      ...
      IFS=' ' read -r -a colors <<< $(get_tmux_option "@dracula-tailupdate-colors" "Red dark_gray")
      script="#($current_dir/tailupdate.sh)"

    elif [ $plugin = "tailexit" ]; then
      IFS=' ' read -r -a colors <<< $(get_tmux_option "@dracula-tailexit-colors" "Red dark_gray")
